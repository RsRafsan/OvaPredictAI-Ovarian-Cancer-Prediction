...
# -----------------------------
# Federated SVM Wrapper (added)
# -----------------------------
class FederatedSVMWrapper:
    """Wrapper to make the federated SVM dictionary behave like a sklearn model with predict_proba."""
...
with st.sidebar:
    st.header("Settings")
    default_model_path = "best_federated_svm_v4.pkl"   # changed from overall_best_federated_xgb.pkl
    ...
    try:
        ...
        # NEW: if it's a dict with the federated SVM keys, wrap it
        if isinstance(model_obj, dict) and 'rff_mapper' in model_obj and 'global_w' in model_obj:
